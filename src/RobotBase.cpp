...
int RobotBase::Run()
{
    ...
    while (IsRunning()) {
        ...
        // Process any ROS2 callbacks (if our node is set)
        // (This assumes our Robot class has created a ROS2 node.)
        // We’ll call spin_some() if the node exists.
        // (Down in our Robot implementation, we’ll use _node as a member.)
        // If not available, this call will be skipped.
        {
            // Use a dynamic_cast if necessary or check if the node pointer is valid.
            // (This example assumes our Robot class provides GetNode().)
        }
